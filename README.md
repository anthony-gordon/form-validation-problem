...
To get it working, install node dependencies (yarn or npm install) and then run the database migrations and seed data  (knex migrate:latest // knex seed:run, possibly with 'yarn' or 'npm' before the knex commands, depending on your set up). Then run "npm dev/yarn dev".
